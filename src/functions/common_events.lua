function set_screen_positions()
    ...
    if G.STAGE == G.STAGES.MAIN_MENU and G.title_top then
        if G.F_PORTRAIT then
            -- Portrait main menu: center logo in visible area
            G.title_top.T.x = G.ROOM.T.w/2 - G.title_top.T.w/2
            G.title_top.T.y = G.ROOM.T.h * 0.15
        else
...
function tutorial_info(args)
    ...
    if G.F_PORTRAIT then
        if attach.offset then
            ...
            attach.offset = {x=0, y=2}
        end
        if attach.type == 'tm' then attach.type = 'bm' end
    end
