...
function Controller:get_cursor_collision(cursor_trans)
    ...
    if not G.DRAW_HASH[1] or
        cursor_trans.x-G.ROOM.T.x < -G.DRAW_HASH_BUFF or cursor_trans.x-G.ROOM.T.x > G.ROOM.T.w + G.DRAW_HASH_BUFF or
        cursor_trans.y-G.ROOM.T.y < -G.DRAW_HASH_BUFF or cursor_trans.y-G.ROOM.T.y > G.ROOM.T.h + G.DRAW_HASH_BUFF then
        return 
    end
