...
function UIElement:click()
    local function _dbg(msg)
        ...
        if love and love.system and love.system.getOS and love.system.getOS() == 'Android' then
            pcall(function()
                local p = io.popen('log -t "BALATRO_UI" "'..msg:gsub('"', '\\"')..'"', 'w')
    ...
    if not self.config then
        _dbg("click() ignored: no config")
    end

    if self.config and self.config.button then
        _dbg(string.format(
            "click() attempt: id=%s button=%s visible=%s under_overlay=%s disable_button=%s last_clicked=%s now=%s",
            tostring(self.config.id),
            tostring(self.config.button),
            tostring(self.states and self.states.visible),
            tostring(self.under_overlay),
            tostring(self.disable_button),
            tostring(self.last_clicked),
            tostring(G and G.TIMERS and G.TIMERS.REAL)
        ))
    end
    ...
    if self.config.button and (not self.last_clicked or self.last_clicked + 0.1 < G.TIMERS.REAL) and self.states.visible and not self.under_overlay and not self.disable_button then
        ...
        if G.OVERLAY_TUTORIAL and G.OVERLAY_TUTORIAL.button_listen == self.config.button then
            _dbg("overlay tutorial intercept: calling tut_next()")
            G.FUNCS.tut_next()
        end

        if not (G and G.FUNCS) then
            _dbg("ERROR: G.FUNCS is nil")
        elseif type(G.FUNCS[self.config.button]) ~= "function" then
            _dbg("ERROR: missing callback for button=" .. tostring(self.config.button))
        else
            _dbg("dispatching callback: " .. tostring(self.config.button))
