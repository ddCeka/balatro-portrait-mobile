...
function CardArea:move(dt)
    ...
    if self == G.hand then 
        -- Early exit if hand is not properly initialized
        if not G.hand or not G.hand.T then return end
        ...
        if G.F_PORTRAIT then
            ...
            desired_y = (G.TILE_H or 10) - G.hand.T.h - 1.9*((not G.deck_preview and (G.STATE == G.STATES.SELECTING_HAND or G.STATE == G.STATES.DRAW_TO_HAND)) and 1 or 0)
        end
        G.hand.T.y = 15*(G.real_dt or dt)*desired_y + (1-15*(G.real_dt or dt))*G.hand.T.y
